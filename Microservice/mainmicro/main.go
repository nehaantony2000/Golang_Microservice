...
func main() {
	...
	usrhandlerobj.Connection("localhost", "postgres", "1234", "Microservice", "5432")
...
func InvHome(w http.ResponseWriter, r *http.Request) {
	client := resty.New()
	resp, err := client.R().Get("localhost:8200/inventory")
...
func ProHome(w http.ResponseWriter, r *http.Request) {
	client := resty.New()
	resp, err := client.R().Get("localhost:8100/product")
	// print the values in the response
	if err != nil {
		panic(err)
	}
...
func SinglePro(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	client := resty.New()
	resp, err := client.R().Get("localhost:8100/product/" + vars["id"])
	// print the values in the response
	if err != nil {
		panic(err)
	}
...
func DelInv(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	client := resty.New()
	resp, err := client.R().Delete("localhost:8200/delinventory/" + vars["id"])
	// print the values in the response
	if err != nil {
		panic(err)
	}
...
func addInv(w http.ResponseWriter, r *http.Request) {
	...
	resp, err := client.R().
		...
		SetBody(string(d)).
		Post("localhost:8200/addinventory")
	// print the values in the response
	if err != nil {
		panic(err)
	}
...
func addPro(w http.ResponseWriter, r *http.Request) {
	...
	resp, err := client.R().
		SetHeader("Content-Type", "application/json").
		SetBody(string(d)).
		Post("localhost:8100/addproduct")
	// print the values in the response
	if err != nil {
		panic(err)
	}
...
func addOrd(w http.ResponseWriter, r *http.Request) {
	...
	resp, _ := client.R().Get("localhost:8200/singleinventory/" + vars["id"])

	// check the product is availabale in resp
	if resp.StatusCode() == 200 {
		resp, err := client.R().Post("localhost:8300/addorder")
		// print the values in the response
		if err != nil {
			panic(err)
		}
